...
const QUICK_QUESTIONS = [
  "What is my notice period for leaves?",
  "How does probation affect my leaves?",
  "Can I apply for half-day WFH?",
...
export default function EmployeePoliciesPage() {
  ...
  const fetchPolicies = async () => {
    try {
      const res = await api.get<PoliciesResponse>("/employee/portal/my-policies");
      setData(res.data);
      // Trigger initial AI explanation
      fetchInitialExplanation();
    } catch (err) {
      console.error(err);
      toast.error("Failed to load your policy details");
      setLoading(false);
    }
  };
  ...
  const handleAskQuestion = async (questionText: string) => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err.response?.data?.message || "Failed to get explanation from AI");
      setChatMessages((prev) => [
        ...prev,
        {
          role: "assistant",
  ...
  const wp = data?.wfhPolicy;
  const balances = data?.leaveBalances || [];
  const wfhUsed = data?.wfhUsedThisMonth ?? 0;
    ...
    <div className="max-w-7xl mx-auto space-y-6 px-1">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
          View your leave and WFH policies line-by-line with AI insights
        </p>
      </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm transition-all hover:shadow-md">
            ...
            {lp ? (
              ...
              <div className="py-6 text-center text-sm text-slate-500">
                No leave rules assigned. Contact your HR administrator.
              </div>
            )}
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm transition-all hover:shadow-md">
            ...
            {wp ? (
              ...
              <div className="py-6 text-center text-sm text-slate-500">
                No WFH policy assigned. Contact your HR administrator.
              </div>
            )}
          </div>
        </div>
