 ...
 * ─────────────────────
 * Edit the templates below, then run:
 *
 *   npm run seed:emails
 *
 * This will UPSERT all templates — creating new ones and updating existing ones.
 * Admin edits made via the UI will be overwritten by this seeder.
 * Run this whenever you want to push template changes to the database.
 */
...
/**
 * WRAP — outer email shell: white card, blue top-bar, header, footer.
 * @param orgName    shown in top-left of header (falls back to "{{orgName}}")
 * @param tagline    small muted text in header below org name
...
<div style="background:${COLOR_SURFACE};padding:32px 0;${FONT}">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0"
    ...
    <tr>
      <td style="padding:24px 32px 20px;border-bottom:1px solid ${COLOR_SURFACE};">
        <p style="margin:0 0 4px 0;font-size:13px;font-weight:700;
                  color:${COLOR_BRAND};letter-spacing:1.2px;
                  text-transform:uppercase;">${orgName}</p>
    ...
    <!-- Footer -->
    <tr>
      <td style="padding:16px 32px;background:${COLOR_SURFACE};
                 border-top:1px solid ${COLOR_BORDER};">
        <p style="margin:0;font-size:11px;color:${COLOR_TEXT_HINT};line-height:1.6;">
          This is an automated notification from <strong style="color:${COLOR_TEXT_MUTED};">${orgName}</strong>
          Leave Planner. Please do not reply to this email.
        </p>
      </td>
    </tr>
...
const ROW = (label: string, val: string, last = false) =>
  `<tr>
    <td style="padding:10px 16px;color:${COLOR_TEXT_MUTED};font-size:13px;
               width:38%;${last ? '' : `border-bottom:1px solid ${COLOR_SURFACE};`}
               vertical-align:top;">${label}</td>
    <td style="padding:10px 16px;color:${COLOR_TEXT_DARK};font-size:13px;
               font-weight:600;${last ? '' : `border-bottom:1px solid ${COLOR_SURFACE};`}
               vertical-align:top;">${val}</td>
  </tr>`;

const TABLE = (rows: string) =>
  `<table role="presentation" width="100%" cellpadding="0" cellspacing="0"
          style="width:100%;background:${COLOR_SURFACE};border:1px solid ${COLOR_BORDER};
                 border-radius:8px;margin:0 0 24px 0;border-spacing:0;">${rows}</table>`;

const SECTION = (title: string) =>
  `<p style="color:${COLOR_TEXT_HINT};font-size:10px;margin:0 0 8px 0;
             text-transform:uppercase;letter-spacing:1.2px;font-weight:700;">${title}</p>`;

const BTN = (label: string, url: string) =>
  `<a href="${url}"
      style="display:inline-block;background:${COLOR_BRAND};color:#ffffff;
...
// ── Badge HTML (used as {{example}} values for preview rendering) ─────────────
// All badge variants — used in variables[].example for UI preview rendering
...
// Export badge map so it can be imported by other modules if needed
export const BADGE_HTML = { BADGE_PENDING, BADGE_APPROVED, BADGE_REJECTED, BADGE_ABSENT, BADGE_CANCELLED };

const SAMPLE_COMMENT  = `
<div style="background:#FFFBEB;border:1px solid #FDE68A;border-left:3px solid #F59E0B;
            border-radius:6px;padding:14px 16px;margin:0 0 24px 0;">
  <p style="margin:0;color:#92400E;font-size:13px;line-height:1.6;">
    <strong>Admin Comment:</strong> Leave rejected due to insufficient notice period.
  </p>
</div>`.trim();
...
const TEMPLATES = [
  ...
  {
    key:         'LEAVE_APPLIED_ADMIN',
    name:        'Leave Request — Admin Notification',
...
`),
  ...
  // ─────────────────────────────────────────────────────────────────────────
  {
    key:         'WFH_APPLIED_ADMIN',
    name:        'WFH Request — Admin Notification',
...
`),
  ...
  // ─────────────────────────────────────────────────────────────────────────
  {
    key:         'LEAVE_CANCELLED_ADMIN',
    name:        'Leave Cancelled — Admin Notification',
...
${TABLE(
  ...
  ROW('From',       '{{fromDate}}') +
  ROW('To',         '{{toDate}}') +
  ROW('Duration',   '{{totalDays}} working day(s)', true)
)}

<div style="margin:0 0 24px 0;">{{statusBadge}}</div>

${DIVIDER()}
<p style="color:${COLOR_TEXT_HINT};font-size:13px;margin:0;">
  No further action is required. The leave balance has been preserved.
</p>
`),
...
${TABLE(
  ...
  ROW('Temporary Password', `<span style="font-family:monospace;font-size:17px;letter-spacing:3px;color:${COLOR_BRAND};font-weight:700;">{{tempPassword}}</span>`, true)
)}

<div style="background:#FEF2F2;border:1px solid #FECACA;border-left:3px solid #EF4444;
            border-radius:0 6px 6px 0;padding:12px 16px;margin:0 0 24px 0;">
...
<div style="text-align:center;background:${COLOR_SURFACE};border:1px solid ${COLOR_BORDER};
            border-radius:10px;padding:28px 24px;margin:0 0 24px 0;">
  <p style="color:${COLOR_TEXT_HINT};font-size:10px;margin:0 0 10px 0;
             text-transform:uppercase;letter-spacing:1.2px;font-weight:700;">Temporary Password</p>
  <p style="font-family:monospace;font-size:30px;letter-spacing:8px;
             color:${COLOR_BRAND};font-weight:700;margin:0;">{{tempPassword}}</p>
</div>

<div style="background:#FEF2F2;border:1px solid #FECACA;border-left:3px solid #EF4444;
            border-radius:0 6px 6px 0;padding:12px 16px;margin:0 0 24px 0;">
  <p style="margin:0;color:#991B1B;font-size:13px;font-weight:600;line-height:1.6;">
    &#9888;&nbsp; Log in immediately and set a new secure password.
  </p>
</div>
...
`),
  ...
  // ─────────────────────────────────────────────────────────────────────────
  {
    key:         'LEAVE_SUBMITTED_EMPLOYEE',
    name:        'Leave Request — Submission Confirmation',
...
`),
  ...
  // ─────────────────────────────────────────────────────────────────────────
  {
    key:         'WFH_SUBMITTED_EMPLOYEE',
    name:        'WFH Request — Submission Confirmation',
...
`),
  ...
  // ─────────────────────────────────────────────────────────────────────────
  {
    key:         'WFH_STATUS_UPDATE',
    name:        'WFH Request — Status Update',
...
`),
  ...
  // ─────────────────────────────────────────────────────────────────────────
  {
    key:         'ADMIN_IMPORTED_LEAVE',
    name:        'Leave Record Added by Admin',
...
<p style="color:${COLOR_TEXT_MUTED};font-size:13px;margin:0 0 24px 0;line-height:1.7;">
  This leave has been marked as approved and your balance has been updated.
